...
if [[ ! -z "$NR_INSTALL_KEY" ]] && [[ ! -f /etc/php/7.3/apache2/conf.d/20-newrelic.ini ]]; then
    [ -z "$NR_APP_NAME" ] && NR_APP_NAME=app@`hostname`
    cat << EOF > /etc/php/7.3/mods-available/newrelic.ini
...
EOF
    ln -s /etc/php/7.3/mods-available/newrelic.ini /etc/php/7.3/apache2/conf.d/20-newrelic.ini
fi
